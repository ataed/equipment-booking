...
// ---------------------------------------------------------------------------
// 1. 1. String constants: booking status, role claim, equipment status
// ---------------------------------------------------------------------------
...
// The custom claim key. Must match request.auth.token.role in firestore.rules.
// Nothing enforces that correspondence except the rules tests.
...
export function slotIdsForBooking(deviceId, startTime, durationHours) {
  // Throws rather than returning an empty list. An undefined durationHours made the
  // loop run zero times, an empty batch commit successfully, and a refused booking
  // keep its slots. No error anywhere.
  if (!ALLOWED_DURATIONS_HOURS.includes(durationHours)) {
    throw new Error(`slotIdsForBooking: bad durationHours ${durationHours}`);
  }
